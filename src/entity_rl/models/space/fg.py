...
class SpaceFg(nn.Module):
    def __init__(self):
        ...
        # Priors
        # Some are buffer and some are not. This is stupid.
        # But for compatibility I have to do so.
        ...
        # self.register_buffer('prior_scale_mean_new',
        # torch.tensor(arch.z_scale_mean_start_value))
        # self.register_buffer('prior_scale_std_new',
        # torch.tensor(arch.z_scale_std_value))
        ...
        self.register_buffer(
            "prior_scale_mean",
            torch.tensor([arch.z_scale_mean_start_value] * 2).view(
                (arch.z_where_scale_dim), 1, 1
            ),
        )
        self.register_buffer(
            "prior_scale_std",
            torch.tensor([arch.z_scale_std_value] * 2).view(
                (arch.z_where_scale_dim), 1, 1
            ),
        )
        self.register_buffer(
            "prior_shift_mean",
            torch.tensor([0.0, 0.0]).view((arch.z_where_shift_dim), 1, 1),
        )
        self.register_buffer(
            "prior_shift_std",
            torch.tensor([1.0, 1.0]).view((arch.z_where_shift_dim), 1, 1),
        )
    ...
    def forward(self, x, globel_step):
        ...
        # (B, 3, H, W) -> (B*G*G, 3, H, W).
        # Note we must use repeat_interleave instead of repeat
...
class ImgEncoderFg(nn.Module):
    ...
    def __init__(self):
        ...
        # Image encoding -> latent distribution parameters
        # (B, 128, G, G) -> (B, D, G, G)
        ...
        # (G, G). Grid center offset. (offset_x[i, j], offset_y[i, j])
        # is the center for cell (i, j)
    ...
    def forward(self, x, tau):
        ...
        # (B, 1, G, G) -> (B, G*G, 1)
        z_depth_mean = reshape(z_depth_mean)
        z_depth_std = reshape(z_depth_std)
        ...
        z_scale_std = reshape(z_scale_std)
        ...
        z_shift_std = reshape(z_shift_std)
...
def reshape(x):
    """(B, D, G, G) -> (B, G*G, D)"""
    B, D, G_0, G_1 = x.size()
    return x.permute(0, 2, 3, 1).view(B, G_0 * G_1, D)
